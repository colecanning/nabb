...
export interface InstagramCrawlError {
  success: false;
  error: string;
  debug?: {
    htmlLength?: number;
 ...
 */
export async function crawlInstagramUrl(url: string): Promise<InstagramCrawlResult> {
  let browser;
  ...
  try {
    ...
    try {
      ...
      throw new Error('Failed to load Instagram page. It may be taking too long to respond.');
    }
    ...
    if (!description) {
      ...
      const error: InstagramCrawlError = {
        success: false,
